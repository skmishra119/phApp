...
define('DROOT','./');
define('XROOT','../../');
require_once '/var/www/html/ts_ptest/settings.inc.php';
$CNR=$st->connectDB();
$SQR="select ts.fromname, ts.fromemail, tr.testid, tr.runid, tr.fullname, tr.emailid, tr.subject, tr.body from t_pt_test ts inner join t_pt_testrun tr on ts.testid=tr.testid where tr.status='SCHEDULED' and tr.activity is null and ts.startdate<='".date('Y-m-d H:i:s')."' and ts.enddate>='".date('Y-m-d H:i:s')."' and ts.process='TESTRUN'";
$RSR=mysqli_query($CNR,$SQR);
while($RWR=mysqli_fetch_assoc($RSR)){
	$mal=$st->sendMail(trim($RWR['fromemail']), trim($RWR['fromname']), trim($RWR['emailid']), trim($RWR['subject']), trim($RWR['body']));
	if(trim($mal)=='SENT'){
		...
		$SQV="update t_pt_test set process='RUNNING' where testid='".trim($RWR['testid'])."' and process='TESTRUN'";
		$RSV=mysqli_query($CNR,$SQV);
	}
}


$SQR="update t_pt_testrun set status='COMPLETED',activity=(case when activity is null then 'PASSED' else activity end), activitydate=(case when activitydate is null then '".date('Y-m-d H:i:s')."' else activitydate end) where testid in (select testid from t_pt_test where  enddate<'".date('Y-m-d H:i:s')."' and process in ('TESTRUN','RUNNING'))";
$RSR=mysqli_query($CNR,$SQR);
if($RSR){
	$SQT="update t_pt_test set process='COMPLETED' where enddate<'".date('Y-m-d H:i:s')."' and process in ('TESTRUN','RUNNING')";
...
$bdy='Dear SYSADMIN,<br/><br/>Thsre are list of the clients need training on phishing, The list is hereunder:';
