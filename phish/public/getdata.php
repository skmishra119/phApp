...
// 		$WHR .= (trim($STATUS)!="" && trim($STATUS)!='UNKNOWN') ? " and r.activity='".trim($STATUS)."'" : ((trim($STATUS)=="UNKNOWN") ? " and r.activity is null" :"" );
		...
		//$FDT=$st->getDataArray("t_pt_testrun tr, t_pt_test ts", "distinct ts.domain, ifnull(tr.activity,'UNKNOWN') as 'status', count(*) as 'tot'", "tr.testid=ts.testid and tr.status in ('RUNNING','COMPLETED')".$WHR.$WHX, "group by tr.activity, ts.domain");
