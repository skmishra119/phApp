    ...
    if(sizeof($st->errors) > 0)
    {
    	...
    	switch(trim($_POST['reptype'])){
    		case "OVERALL":
    			...
    			break;
    		case "TOTAL_TEST":
    			echo $st->addChartScript("TOTAL_TEST", trim($CID), $DTS,  "chtReport", "pie", "Campaign Synopsis", "Result: <b>{point.y}</b> ({point.percentage:.1f}%)",true);
    			$REPORT_DATA = $st->showDBDataInfo("TOTAL_TEST",$USERINFO,$DTS,trim($CID),true,"","","",true);
    			break;
    		case "ALL_TEST":
    			echo $st->addChartScript("TEST_ANALYSIS", trim($CID), $DTS,  "chtReport", "pie", "Campaign report", "Result: <b>{point.y}</b> ({point.percentage:.1f}%)",true);
    			$REPORT_DATA = $st->showDBDataInfo("TEST_ANALYSIS",$USERINFO,$DTS,trim($CID),true,"","","",true);
    			break;
    		case "TEST_DOMAIN":
    			echo $st->addChartScript("DOMAIN_TEST_ANALYSIS", trim($CID), $DTS, "chtReport", "column", "Domain wise campaign report", "Result: <b>{point.y}</b> ({point.percentage:.1f}%)",true);
    			$REPORT_DATA = $st->showDBDataInfo("DOMAIN_TEST_ANALYSIS",$USERINFO,$DTS,trim($CID),true,"","","",true);
    			break;
    		case "TEST_USER_EVENT":
    			echo $st->addChartScript("USER_EVENT_TEST_ANALYSIS", trim($CID), $DTS, "chtReport", "pie", "Event wise campaign report", "Result: <b>{point.y}</b> ({point.percentage:.1f}%)", true);
...
<div class="content_cont_wrapper">
	<div class="header_cont"><h3>Reports</h3></div>
