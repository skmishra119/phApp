...
/**
    ...
    private function validateString($var, $msg, $min=0, $max=0, $options=false, $required=false)
    {
        ...
        {
            if(is_array($options)){
            	$arSrs=explode(', ',$this->source[$var]);
            	foreach ($arSrs as $val){
					if(in_array(trim($val),$options)===false)
        			{
            			$this->errors[$var] = 'Invalid '.$msg . ' selected';
        			}
            	}
    ...
    public function no_csrf($tokeid, $posttoken)
    {
    	if($tokeid!=$posttoken)
    	{
    		unset($_SESSION['token_id']);
    		$_SESSION['token_id']='';
    		$this->errors['msg'] = 'Unauthorized access.';
    		return $this->errors;
    	}
    }
